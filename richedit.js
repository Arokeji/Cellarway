...
//---------------------------------------------------------------------------------------------------------
  ...
  var txtHyperlink       = "Añadir enlace";
  var txtCut             = "Cut";
  var txtCopy            = "Copy";
  var txtPaste           = "Paste";
  var txtUndo            = "Undo";
...
// Configuration
//---------------------------------------------------------------------------------------------------------
  this.editorBGColor = "#006666";            // editor background color
  this.editorBorder = "0px groove";             // editor border (CSS-spec: "size style color")

  this.textWidth = 390;                         // text field width (pixels)
  this.textHeight = 120;                        // text field height (pixels)
  this.textBGColor = "white";                   // text field background color
  this.textBorder = "1px solid green";                // text field border (CSS-spec: "size style color")
...
//---------------------------------------------------------------------------------------------------------
  ...
  this.initEditor = function(content) {
    ...
    else alert("Tu navegador no soporta el texto enriquecido.");
  }
  ...
  this.createImage = function() {
    ...
    if(f.URL.value && f.URL.value != 'http://') {
      if(this.curSelection) this.doCmd('insertHTML', '<img height="20" width="20" src="' + f.URL.value + '">');
      else this.doCmd('insertHTML', '<img src="' + f.URL.value + '">');
    }
  }
  ...
  this.buildEditor = function() {
    document.writeln('<style> ' +
					 ...
					 'background-image:url(images/backcomentario.jpg); ' +
                     'width: 154%; ' +
                     'margin: 0px; ' +
                     'padding: 4px; ' +
                     ...
                     '.cssToolBar' + this.id + ' { ' +
                     'background-color: ' + this.editorBGColor + '; ' +
                     'border: 0px solid ' + this.editorBGColor + '; ' +
                     'padding: 2px; ' +
                     '} ' +
                     '.cssRaised' + this.id + ' { ' +
    ...
    document.writeln('<div id="rtoToolBar' + this.id + '">');//aqui se puede cambiar el fondo y el borde
	
	document.writeln('Nombre:<br />');
	
	document.writeln('<input name="nombrecomentario" type="text" size="30" maxlength="30" />');
	
    document.writeln('<table border=0 cellspacing=0 cellpadding=0 align=center><tr align=center>');
	...
	document.writeln('<td><table border=0 cellspacing=0 cellpadding=0><tr>');
  ...
  this.barGraph = function(values, labels, bColor, lColor, showVal, legend, bSize, bBorder, bLen) {
    ...
    if(!bBorder) bBorder = '1px outset white';
    if(!bSize) bSize = 15;
    if(!bLen) bLen = 1.0;
    ...
    var spacer = maxSize = Math.round(mPerc * mul + valSpace + border * 1);

    if(max_neg) {
      var mPerc_neg = sum ? Math.round(-max_neg * 100 / sum) : 0;
      var spacer_neg = Math.round(mPerc_neg * mul + valSpace + border * 1);
      maxSize += spacer_neg;
    }
    ...
    if(legend) {
      graph += '</td><td width=10>&nbsp;</td><td><div style="padding:4px; background-color:#F0F0F0; border:0px solid #808080">';
...
/*HASTA AQUI LA FUNCION DE INSERTAR TEXTO*/
...
if((IE || GK) && DM) {
  ...
  document.writeln('<tr><td class="cssFont2" nowrap>URL:</td><td>&nbsp;<input type=text name="URL" size=50 maxlength=800 class="cssFormField"></td></tr>');
